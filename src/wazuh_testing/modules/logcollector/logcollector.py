...
from . import PREFIX
from wazuh_testing import constants
...
DEFAULT_AUTHD_REMOTED_SIMULATOR_CONFIGURATION = {
    ...
    'client_keys': os.path.join(constants.paths.WAZUH_PATH, 'etc', 'client.keys'),
    'server_keys': os.path.join(constants.paths.WAZUH_PATH, 'etc', 'sslmanager.key'),
    'server_cert': os.path.join(constants.paths.WAZUH_PATH, 'etc', 'sslmanager.cert'),
    'authd_port': 1515,
    'remoted_port': 1514,
    'protocol': 'tcp',
...
def callback_read_macos_message(msg):
    return callbacks.make_callback(pattern=msg, prefix=prefix)
...
def callback_analyzing_file(file):
    ...
    return callbacks.make_callback(pattern=msg, prefix=prefix, escape=True)
...
def callback_read_lines(command, escape=False):
    ...
    return callbacks.make_callback(pattern=msg, prefix=prefix, escape=escape)
...
def callback_event_log_service_down(location, severity='WARNING'):
    ...
    return callbacks.make_callback(pattern=log_format_message, prefix=prefix)
...
def get_data_sending_stats(log_path, socket_name):
    ...
    if not os.path.isfile(constants.paths.variables.LOGCOLLECTOR_STATISTICS_FILE):
        raise FileNotFoundError

    with open(constants.paths.variables.LOGCOLLECTOR_STATISTICS_FILE, 'r') as json_file:
        data = load(json_file)
...
def get_next_stats(current_stats, log_path, socket_name, state_interval):
    ...
    Raises:
        FileNotFoundError: If the next statistics could not be obtained according to the interval
                           defined by "logcollector.state_interval".
    """
    mtime_current = os.path.getmtime(constants.paths.variables.LOGCOLLECTOR_STATISTICS_FILE)
    ...
    for _ in range(state_interval * 2):
        ...
        mtime_next = os.path.getmtime(constants.paths.variables.LOGCOLLECTOR_STATISTICS_FILE)
...
def callback_logcollector_started():
    ...
    return callbacks.make_callback(pattern='Started', prefix=prefix)


def callback_log_bad_predicate():
    """Check for the macOS ULS bad predicate message."""
    return callbacks.make_callback(pattern="Execution error 'log:", prefix=prefix)


def callback_macos_uls_log(expected_message):
    """Callback function to wait for a macOS' ULS log, collected by logcollector."""
    return callbacks.make_callback(pattern=expected_message, prefix=prefix, escape=False)


def callback_logcollector_log_stream_log():
    """Check for logcollector's macOS ULS module start message."""
    return callbacks.make_callback(pattern='Monitoring macOS logs with:(.+?)log stream',
                                    prefix=prefix, escape=False)


def callback_file_status_macos_key():
    """Check for 'macos' key."""
    return callbacks.make_callback(pattern='"macos"', prefix='.*', escape=False)


def callback_log_macos_stream_exit():
    """Check for the macOS ULS log stream exit message."""
    return callbacks.make_callback(pattern="macOS 'log stream' process exited, pid:", prefix=prefix)
...
def wait_statistics_file(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT):
    ...
    for _ in range(timeout):
        if os.path.isfile(constants.paths.variables.LOGCOLLECTOR_STATISTICS_FILE):
            break
        else:
            sleep(1)
