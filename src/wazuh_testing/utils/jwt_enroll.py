...
The frozen vectors this module is pinned against are the product's own, not this
implementation's: wazuh/wazuh: src/shared_modules/utils/jwt/testVectors.hpp, namespaces
``test_vectors::enroll`` and ``::enroll_token``. Change a derivation here and it stops matching
the manager, which is the only thing that matters.
"""
import base64
import hashlib
import hmac
