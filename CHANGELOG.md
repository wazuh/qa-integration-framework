...
| [#39321](https://github.com/wazuh/wazuh/issues/39321) | Added the `ca_generation` CA bundle publication to `RemotedSimulator`'s notify response. |
| [#39321](https://github.com/wazuh/wazuh/issues/39321) | Added the `Wazuh-CA-Generation` header to `GET /cacerts`, with `cacerts_generation` to set it apart from what notify advertises. |
| [#39321](https://github.com/wazuh/wazuh/issues/39321) | Added the `rate_limited` outcome to `cacerts_force_error`, answering `429` with a `Retry-After`. |
