...
| [#39284](https://github.com/wazuh/wazuh/issues/39284) | The default API password is resolved instead of assumed: `login()`, `set_authorization_header()` and `get_api_details_dict()` take it from the variable that user's password is installed through, `INITIAL_WAZUH_PASSWORD` for `wazuh` and `INITIAL_WAZUH_WUI_PASSWORD` for `wazuh-wui`, then from the credentials file the node was seeded from (`api/configuration/security/wazuh-preseeded-passwords.yml`, readable by root and the Wazuh group), and only then fall back to the historical literal, which still applies to a 4.x manager. One variable per user, not a shared one: an installation that supplies `wazuh` and lets `wazuh-wui` be generated would otherwise authenticate `wazuh-wui` with the other user's password. A 5.x manager ships no password for its default users, and `rbac.db` holds only the hash. Resolution happens at call time rather than as a default argument, since the environment is provisioned after this module is imported, and only for the default users: a user a suite creates itself keeps the password its own fixture stored |
