...
| [#39321](https://github.com/wazuh/wazuh/issues/39321) | Added the CA bundle publication to `RemotedSimulator`: `ca_generation` is advertised top-level on every notify, and `GET /cacerts` stamps the bundle it serves with the `Wazuh-CA-Generation` header, so an agent's adopt-only-what-was-advertised rule can be driven end to end |
| [#39321](https://github.com/wazuh/wazuh/issues/39321) | Added `cacerts_generation`, which follows `ca_generation` unless set apart, so a test can play a node behind a load balancer answering with a bundle other than the one just advertised, or one that vouches for nothing at all |
| [#39321](https://github.com/wazuh/wazuh/issues/39321) | Added the `rate_limited` outcome to `cacerts_force_error`, answering `429` with a `Retry-After`, so the rate limit a rotating fleet meets in the field can be driven against the agent's backoff |
